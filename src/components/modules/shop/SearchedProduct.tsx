...
function SearchedProduct(props: productBoxType) {
  ...
  return (
    <Link
      ...
      className="searched-product flex w-full bg-new-products py-3 max-h-[70px] items-center justify-between px-4 border-b border-b-[#e1e1e1]"
    >
      <div className="image-wrapper w-[60px] h-[60px] flex items-center justify-center my-auto -mt-4">
        <Image src={sources[0]} width={400} height={400} alt={title} className="w-full h-full flex items-center justify-center"/>
      </div>
      <div className="searched-product__infos text-left min-w-[150px]">
        <h4 className="searched-product__title font-bold">{title}</h4>
        <div className="searched-product__prices flex items-center gap-x-3">
          {discount ? (
            <>
